...
Sometimes you need to make an async decision (API call, feature flag check, etc.) before knowing which screen to show. The `createRouterScreen` factory enables this pattern.
...
```tsx
// Factory creates a screen component from a decision hook
...
1. **Define your decision hook** - contains the async logic:
...
function useCheckoutDecision(params: { cartId: string }) {
  ...
  useEffect(() => {
    async function decide() {
      const eligibility = await checkEligibility(params.cartId);
      setState({
        isLoading: false,
...
2. **Create the router screen**:

```tsx
const CheckoutRouter = createRouterScreen(useCheckoutDecision);
```

3. **Register in your stack** - use the attached `screenOptions`:
