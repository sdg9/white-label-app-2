...
Sometimes you need to make an async decision (API call, feature flag check, etc.) before knowing which screen to show. The `@wla/platform-navigation` library provides two factory functions for this pattern.
...
Each router component has a `screenOptions` property attached. Use it when registering the screen:

```tsx
import { createRouterScreen } from '@wla/platform-navigation';
...
You can also import `routerScreenOptions` directly if needed:

```tsx
import { routerScreenOptions } from '@wla/platform-navigation';
```
